...
- (id)initWithFrame:(CGRect)frame andLeftItems:(NSArray *)leftItems andRightItems:(NSArray *)RightItems{
    ...
    if (self) {
        self.frame = CGRectMake(0, BUTTON_HEIGHT, frame.size.width, frame.size.height - STATUS_AND_NAVIGATION_HEIGHT - 40);
...
- (void)initSecondTableViewWithFrame:(CGRect)frame {
    _secondTableView = [[UITableView alloc] initWithFrame:CGRectMake(frame.size.width / 2, 0, frame.size.width/2, frame.size.height / 2) style:UITableViewStylePlain];
    ...
    if (tableView == _firstTableView) {
        ...
        switch (indexPath.section) {
            case 0:
            {
              ...
              //  NSar *dic = [array objectAtIndex:indexPath.row];
        ...
        UIImageView *accessoryView = [[UIImageView alloc] initWithImage:[[UIImage imageNamed:@"xuanzhong"] imageWithColor:[UIColor spBigRedColor]]];
        accessoryView.right = cell.contentView.width - 10;
        accessoryView.top = 15.f;
        ...
        noSelectLine.backgroundColor = [UIColor grayColor];
        noSelectLine.alpha = 0.5;
        [noSelectView addSubview:noSelectLine];
        
        UIView *selectView = [[UIView alloc] initWithFrame:cell.contentView.bounds];
        [selectView addSubview:accessoryView];
        ...
        selectLine.backgroundColor = [UIColor grayColor];
...
- (void)showSingleOrDouble {
    if (_leftArray.count <= 0) {
        ...
        _secondTableView.frame = CGRectMake(SCREEN_WIDTH / 2, 0, SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2);
    }
}
