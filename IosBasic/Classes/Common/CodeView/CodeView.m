...
- (instancetype)initWithFrame:(CGRect)frame
    ...
    if (self) {
        self.frame = frame;
        self.backgroundColor = [UIColor black1];
        _textArray = [NSMutableArray arrayWithCapacity:num];
        
        lineNum = num;
        //数字样式是的颜色和线条颜色相同
        linecolor = lColor;
        textcolor = tColor;
        lineAnimationColor = lAColor;
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    ...
    NSInteger length = _textArray.count;
...
- (void)addLineAnimation {
    ...
    if (_textArray.count>0) {
        CAShapeLayer *line = [CAShapeLayer layer];
        line.fillColor = [lineAnimationColor CGColor];
        NSInteger i = _textArray.count - 1;
        UIBezierPath *path = [UIBezierPath bezierPathWithRect:CGRectMake(Space * (2 *i + 1) + i * LineWidth, self.frame.size.height - LineBottomHeight, LineWidth, LineHeight)];
        line.path = path.CGPath;
        line.hidden = NO;
        [self.layer addSublayer:line];
        ...
        shakeAnimation.fromValue = [NSNumber numberWithFloat:-3];
        shakeAnimation.toValue = [NSNumber numberWithFloat:3];
        shakeAnimation.autoreverses = YES;
        [line addAnimation:shakeAnimation forKey:nil];
