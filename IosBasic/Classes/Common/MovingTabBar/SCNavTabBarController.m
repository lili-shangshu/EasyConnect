...
- (id)initWithtabBarColor:(UIColor *)Color{
   
    self = [super init];
    if (self)
    {
        _navTabBarColor = Color;
    }
    return self;
}
...
- (void)initConfig
{
    ...
    if (!_navTabBarColor) {
        _navTabBarColor = [UIColor managerNavgationBarColor];
    }
...
- (void)viewInit
{
    ...
    _navTabBar = [[SCNavTabBar alloc] initWithFrame:CGRectMake(DOT_COORDINATE, DOT_COORDINATE, SCREEN_WIDTH, NAV_TAB_BAR_HEIGHT) showArrowButton:_showArrowButton withbarColor:_navTabBarColor];
    _navTabBar.delegate = self;
    
    _navTabBar.bgColor = _navTabBarColor;
    _navTabBar.lineColor = _navTabBarLineColor;
    _navTabBar.itemTitles = _titles;
    _navTabBar.arrowImage = _navTabBarArrowImage;
    [_navTabBar updateData];
//    _navTabBar selectButtonAtIndex:
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [_navTabBar selectButtonAtIndex:self.selectedIndex];
}
...
//    _navTabBar.currentItemIndex = _currentIndex;
    ...
    [_navTabBar updateLinePositionWithCurrentIndexFloat:(scrollView.contentOffset.x / SCREEN_WIDTH)];
    
}

- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    if ([self.delegate respondsToSelector:@selector(willSelectIndex:viewController:)]) {
        [self.delegate willSelectIndex:_currentIndex viewController:self.subViewControllers[_currentIndex]];
    }
    _navTabBar.currentItemIndex = _currentIndex;
    NAPostNotification(@"scrollTop", nil);
}
