...
{
    //解析文本
    NSString *analyzedString = _text;
    if (_nameString) analyzedString = [NSString stringWithFormat:@"%@：%@",_nameString,_text];
    _textAnalyzed = [self analyzeText:analyzedString];
    
    if (_nameString) {
        NSMutableString *cString = [NSMutableString stringWithString:_textAnalyzed];
        [cString deleteCharactersInRange:[cString rangeOfString:[NSString stringWithFormat:@"%@：",_nameString]]];
        _textAnalyzed = [NSString stringWithString:cString];
    }
    ...
    if (_nameString) {
        ...
        NSDictionary *attDic = @{NSForegroundColorAttributeName : [UIColor spCommentUserNameColor],
                                 NSFontAttributeName:[UIFont systemFontOfSize:self.font.pointSize],
                                 NSParagraphStyleAttributeName:paragraphStyle};
        attributedText = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@：",_nameString] attributes:attDic];
        [attString insertAttributedString:attributedText atIndex:0];
    }
...
- (CGSize)contentSize
{
    ...
    if (_nameString) {
        [string appendString:_nameString];
    }
    
    [string appendString:_text];
    ...
    CGRect contentFrame = [stripStr boundingRectWithSize:CGSizeMake(CGRectGetWidth(self.bounds), MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading attributes:@{ NSFontAttributeName : self.font } context:nil];
    
    return CGSizeMake(contentFrame.size.width, contentFrame.size.height > 25.f? contentFrame.size.height*1.1 :contentFrame.size.height);
}

- (NSMutableAttributedString *)resetNameColor
{
    NSMutableAttributedString *attributedText;
...
+ (NSString *)shortenEmotionString:(NSString *)string
{
    if ([string rangeOfString:@"["].location == NSNotFound) {
//        NALog(@"-------> No Emotion ------");
        return string;
    }
    NSArray *emotionArray = [NAEmotionsView shareView].emotionArray;
    for (NSString *emString in emotionArray)
    {
        string = [string stringByReplacingOccurrencesOfString:emString withString:@"d我"];
    }
...
+ (CGFloat)heightWithText:(NSString *)string width:(CGFloat)width font:(UIFont *)font
{
    ...
    return contentFrame.size.height > 25.f? contentFrame.size.height*1.1 :contentFrame.size.height;
}
