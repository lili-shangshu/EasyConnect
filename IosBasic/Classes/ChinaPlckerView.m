...
- (void)initPickerView{
    ...
    // 默认
    self.selectedProvinceID = @"5";  // 北京市
    self.selectedCityID = @"14";     // 北京市
    self.selectedAreaID = @"37";    // 东城区
    
    self.chinaModel.provinceModel = ((ECProvinceModel *)[self.chinaModel getProvinceDataByID:self.selectedProvinceID]);
    self.chinaModel.cityModel = ((ECCityModel *)[self.chinaModel getCityDataByID:self.selectedCityID]);
    self.chinaModel.areaModel = ((ECAreaModel *)[self.chinaModel getAreaDataByID:self.selectedAreaID]);
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    if (component == 0) { // 省份
        ...
        textLabel.text = ((ECProvinceModel *)[self.chinaModel getAllProvinceData][row]).name;
        ...
        textLabel.frame = CGRectMake(0, 0, (kScreenWidth - 20)/3, ROW_HEIGHT);
        textLabel.textAlignment = NSTextAlignmentCenter;
        textLabel.text = ((ECCityModel *)[self.chinaModel getCityDataByParentID:self.selectedProvinceID][row]).name;
        textLabel.font = [UIFont systemFontOfSize:14];
        textLabel.textColor = COLOR(51, 51, 51, 1);
        [view addSubview:textLabel];
        return view;
        ...
        textLabel.frame = CGRectMake(0, 0, (kScreenWidth - 20)/3, ROW_HEIGHT);
        textLabel.textAlignment = NSTextAlignmentCenter;
        textLabel.text = ((ECAreaModel *)[self.chinaModel getAreaDataByParentID:self.selectedCityID][row]).name;
        textLabel.font = [UIFont systemFontOfSize:14];
        textLabel.textColor = COLOR(51, 51, 51, 1);
        [view addSubview:textLabel];
        return view;
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    switch (component) {
        case 0:{ // 选择省份
            self.selectedProvinceID = ((ECProvinceModel *)[self.chinaModel getAllProvinceData][row]).idNumber;
            [pickerView reloadComponent:1]; // 重载城市
            [pickerView selectRow:0 inComponent:1 animated:YES];
            self.chinaModel.provinceModel = (ECProvinceModel *)[self.chinaModel getAllProvinceData][row];
            // 选择了省份就自动定位到该省的第一个市
            self.selectedCityID = ((ECCityModel *)[self.chinaModel getCityDataByParentID:self.selectedProvinceID][0]).idNumber;
            self.chinaModel.cityModel = (ECCityModel *)[self.chinaModel getCityDataByParentID:self.selectedProvinceID][0];
            ...
            if ([self.chinaModel getAreaDataByParentID:self.selectedCityID].count > 0) {
                self.chinaModel.areaModel = (ECAreaModel *)[self.chinaModel getAreaDataByParentID:self.selectedCityID][0];
            }
            else{
                self.chinaModel.areaModel.name = @"";
                self.chinaModel.areaModel.idNumber = @"";
                self.chinaModel.areaModel.parentId = @"9";
            }
            break;
        }
        case 1:{ // 选择城市
            self.selectedCityID = ((ECCityModel *)[self.chinaModel getCityDataByParentID:self.selectedProvinceID][row]).idNumber;
            [pickerView  reloadComponent:2]; // 重载区域
            [pickerView selectRow:0 inComponent:2 animated:YES];
            self.chinaModel.cityModel = (ECCityModel *)[self.chinaModel getCityDataByParentID:self.selectedProvinceID][row];
            // 选择了市就定位到该市的第一个区
            // 海南 有的市没有区  真是坑啊
            if ([self.chinaModel getAreaDataByParentID:self.selectedCityID].count > 0) {
                self.chinaModel.areaModel = (ECAreaModel *)[self.chinaModel getAreaDataByParentID:self.selectedCityID][0];
        ...
        case 2: // 选择区域
            // 海南 有的市没有区  真是坑啊
            if ([self.chinaModel getAreaDataByParentID:self.selectedCityID].count > 0) {
                self.chinaModel.areaModel = (ECAreaModel *)[self.chinaModel getAreaDataByParentID:self.selectedCityID][row];
            }
            else{
                self.chinaModel.areaModel.name = @"";
                self.chinaModel.areaModel.idNumber = @"";
...
-(void)showChinaPickerView
{
    ...
    [UIView animateWithDuration:0.25f animations:^{
        float pikceY =  kScreenHeight - IT_KEYBOARD_HEIGHT-kNavigationHeight;
