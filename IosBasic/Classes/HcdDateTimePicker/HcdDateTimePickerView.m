...
-(instancetype)initWithDatePickerMode:(DatePickerMode)datePickerMode defaultDateTime:(NSDate*)dateTime isShowASAP:(BOOL)ishow{
    self = [super init];
    if (self) {
        self.defaultDate = dateTime;
        _isShow = ishow;
...
- (void)setTimeBroadcastView
{
    ...
    ASAPBtn = [[UIButton alloc]initWithFrame:CGRectMake((kScreen_Width-60)/2, 0, 60, kTopViewHeight)];
    [ASAPBtn setTitleColor:_buttonTitleColor forState:UIControlStateNormal];
    ASAPBtn.titleLabel.font = [UIFont systemFontOfSize:14];
    [ASAPBtn setBackgroundColor:[UIColor clearColor]];
    [ASAPBtn setTitle:@"ASAP" forState:UIControlStateNormal];
    [ASAPBtn addTarget:self action:@selector(selectedButtons:) forControlEvents:UIControlEventTouchUpInside];
    ASAPBtn.tag = 102;
    if (_isShow) {
        [self addSubview:ASAPBtn];
    }
...
-(void)selectedButtons:(UIButton *)btns{
    ...
    [self dismissBlock:^(BOOL Complete) {
        ...
        }else if (btns.tag == 102){
             weak.clickedASAPBtn(@"ASAP");
        }
        else {
            weak.clickedCancleBtn(@"");
        }
    }];
